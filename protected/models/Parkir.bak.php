...
class Parkir extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('jenis_usaha_id, kelurahan_parkir, kecamatan_parkir, status_sewa, user_id, kuasa_id', 'numerical', 'integerOnly'=>true),
			...
			array('foto_pemohon_upl, ktp_pemohon_upl, surat_izin_upl, npwd_upl, surat_izin_tempat_upl, imb_upl, denah_parkir_upl, daftar_petugas_upl, surat_kuasa_upl, akte_pendirian_perusahaan_upl, jaminan_asuransi_upl', 'length', 'max'=>1024),
                        //array('foto_pemohon_upl, ktp_pemohon_upl, surat_izin_upl, npwd_upl, surat_izin_tempat_upl, imb_upl, denah_parkir_upl, daftar_petugas_upl, surat_kuasa_upl, akte_pendirian_perusahaan_upl, jaminan_asuransi_upl', 'file', 'types'=>'jpg, gif, png'),
                        //array('foto_pemohon_upl, ktp_pemohon_upl, surat_izin_upl, npwd_upl, surat_izin_tempat_upl, imb_upl, denah_parkir_upl, daftar_petugas_upl, surat_kuasa_upl, akte_pendirian_perusahaan_upl, jaminan_asuransi_upl', 'unsafe'),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, nama_perusahaan, alamat_perusahaan, telp_perusahaan, badan_hukum_perusahaan, jenis_usaha_id, lokasi_parkir, jalan_parkir, kelurahan_parkir, kecamatan_parkir, luas_parkir, luas_gedung, daya_tampung_halaman_r2, daya_tampung_halaman_r4, daya_tampung_gedung_r2, daya_tampung_gedung_r4, jam_parkir_1_mulai, jam_parkir_1_selesai, jam_parkir_2_mulai, jam_parkir_2_selesai, jml_petugas_parkir, status_sewa, foto_pemohon_upl, ktp_pemohon_upl, surat_izin_upl, npwd_upl, surat_izin_tempat_upl, imb_upl, denah_parkir_upl, daftar_petugas_upl, surat_kuasa_upl, akte_pendirian_perusahaan_upl, jaminan_asuransi_upl, user_id, kuasa_id', 'safe', 'on'=>'search'),
		);
	}
	 ...
	 */
        ...
        public function beforeSave() {
            ...
            if(parent::beforeSave() && $this->isNewRecord) {
                ...
                //$this->foto_pemohon_upl = $this->user->no_ktp . '.JPG';
                //$this->password = md5($this->no_paket);
                //$this->username = $this->no_ktp;
        ...
        {
            $data = Parkir::model()->findByPk($this->id);
            $data->foto_pemohon_upl = $this->user->no_ktp . '.JPG';
            $data->save(false);
            parent::afterSave();
        }
        */
	...
	public function attributeLabels()
	{
		return array(
			...
			'foto_pemohon_upl' => 'Foto Pemohon',
			'ktp_pemohon_upl' => 'Ktp Pemohon',
			'surat_izin_upl' => 'Surat Izin Lama',
			'npwd_upl' => 'NPWD',
			'surat_izin_tempat_upl' => 'Surat Izin Tempat',
			'imb_upl' => 'Surat Izin Mendirikan Bangunan',
			'denah_parkir_upl' => 'Denah Parkir',
			'daftar_petugas_upl' => 'Daftar Petugas Parkir',
			'surat_kuasa_upl' => 'Surat Kuasa',
			'akte_pendirian_perusahaan_upl' => 'Akte Pendirian Perusahaan',
			'jaminan_asuransi_upl' => 'Jaminan Asuransi',
			'user_id' => 'User',
			'kuasa_id' => 'Kuasa',
		);
	}
